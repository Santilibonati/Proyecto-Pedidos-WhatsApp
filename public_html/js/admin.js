...
async function exportOrdersToExcel(exportType = "current_month", buttonEl = null) {
  ...
  if (filteredOrders.length === 0) {
    ...
    showWarningMessage("No hay pedidos para exportar en el período seleccionado");
    return
  }
  ...
  try {
    ...
    const slimOrders = filteredOrders.map((order) => {
      return {
        ...
        items: (order.items || []).map((item) => ({
          ...
          //detail: item.flavors || null // acá entra el detalle de gustos (si aplica)
          detail: item.flavorsList || null
        })),
      }
    })
    ...
    form.method = "POST"
    form.action = "http://localhost:3000/export_orders.php"
    ...
    form.submit() /*CAMBIAR ENVIO DE DATOS - PARA MAS ADELANTE */
    document.body.removeChild(form)

    //alert("Archivo Excel generado y descargado exitosamente!")
    showSuccessMessage("Archivo Excel generado y descargado exitosamente!")
  } catch (error) {
    //console.error("Error exporting to Excel:", error)
    //alert("Error al generar el archivo Excel: " + error.message)
    showWarningMessage("Error al generar el archivo Excel: " + error.message)
  } finally {
    if (buttonEl) {
...
🍦 Gustos más vendidos este mes:
${Object.entries(todayStats.topFlavors)
  .sort(([,a],[,b]) => b-a)
  .slice(0,3)
  .map(([p,c]) => `• ${p}: ${c} u.`)
  .join("\n")}
